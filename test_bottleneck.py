...
def search_optimized_lowmem(queries, cluster_centers, data_inverted_index, data, k=100,
                            batch_size=50, candidate_chunk_size=50000):
    """
    低内存优化版本：
    1. 增量维护Top-K结果，避免存储完整分数矩阵
    2. 动态调整分块大小，防止单个分块占用过多显存
    """
    device = queries.device
    nq, d = queries.shape
    num_clusters = cluster_centers.size(0)

    # Step 1-3: 计算主次中心（保持原有优化）
    with torch.no_grad():
        inner_products = torch.matmul(queries, cluster_centers.T)
        primary_center_indices = torch.argmax(inner_products, dim=1)
        primary_centers = cluster_centers[primary_center_indices]
    ...
    combined_keys = primary_center_indices * num_clusters + secondary_center_indices
    unique_keys, inverse_indices = torch.unique(combined_keys, return_inverse=True)
    unique_combos = torch.stack([unique_keys // num_clusters, unique_keys % num_clusters], dim=1)
    ...
    # 预取所有组合的候选集
    all_merged_candidates = []
    for c1, c2 in unique_combos:
        c1_list = data_inverted_index.get(c1.item(), torch.tensor([], device=device, dtype=torch.long))
        c2_list = data_inverted_index.get(c2.item(), torch.tensor([], device=device, dtype=torch.long))
        merged = torch.unique(torch.cat([c1_list, c2_list]))
        all_merged_candidates.append(merged)

    # 批量处理多个组合
    num_combos = unique_combos.size(0)
    for combo_start in range(0, num_combos, batch_size):
        combo_end = min(combo_start + batch_size, num_combos)
        current_combos = unique_combos[combo_start:combo_end]
        current_candidates = all_merged_candidates[combo_start:combo_end]

        # 收集当前批次的所有候选点（去重）
        candidate_indices = torch.unique(torch.cat(current_candidates)) if current_candidates else torch.tensor([],
                                                                                                                device=device)
        if candidate_indices.numel() == 0:
            continue

        # 获取候选向量（一次性加载）
        candidate_vectors = data[candidate_indices]  # (total_candidates, d)
        num_candidates = candidate_vectors.size(0)

        # 收集查询组
        query_masks = [inverse_indices == (combo_start + i) for i in range(combo_end - combo_start)]
        grouped_queries = [queries[mask] for mask in query_masks]
        valid_group_indices = [i for i, gq in enumerate(grouped_queries) if gq.size(0) > 0]
        valid_groups = [grouped_queries[i] for i in valid_group_indices]

        if not valid_groups:
            continue

        # 初始化Top-K存储（每个查询组独立维护）
        batch_size = len(valid_groups)
        current_topk_scores = [torch.full((gq.size(0),), float('-inf'), device=device) for gq in valid_groups]

        current_topk_indices = [torch.full((gq.size(0),), -1, dtype=torch.long, device=device) for gq in valid_groups]
        ...
        chunk = candidate_vectors[start:end]  # (chunk_size, d)

        # 混合精度计算
        with autocast():
        # 批处理计算：每个查询组与当前分块的分数
            chunk_scores = [torch.matmul(gq, chunk.T) for gq in
                            valid_groups]  # 每个元素是 (num_queries_in_group, chunk_size)

        # 增量合并Top-K
        for i in range(len(valid_groups)):
            scores = chunk_scores[i]  # (m, chunk_size)
        combined_scores = torch.cat([current_topk_scores[i].unsqueeze(1), scores], dim=1)  # (m, k + chunk_size)

        # 取当前Top-K
        current_k = min(k, combined_scores.size(1))
        topk_scores, topk_indices = torch.topk(combined_scores, current_k, dim=1)

        # 转换为全局候选索引
        global_indices = torch.full_like(topk_indices, -1)
        global_indices[topk_indices < current_k] = current_topk_indices[i][topk_indices < current_k]
        global_indices[topk_indices >= current_k] = candidate_indices[
            start + topk_indices[topk_indices >= current_k] - current_k]

        # 更新当前Top-K
        current_topk_scores[i] = topk_scores
        current_topk_indices[i] = global_indices

        # 及时释放中间变量
        del chunk_scores, scores, combined_scores
        torch.cuda.empty_cache()

        # 存储最终结果
        for i, group_idx in enumerate(valid_group_indices):
            mask = query_masks[group_idx]
        valid_k = min(k, current_topk_indices[i].size(1))

        all_nearest_neighbors[mask, :valid_k] = current_topk_indices[i][:, :valid_k]
        all_top_k_distances[mask, :valid_k] = current_topk_scores[i][:, :valid_k]
...
def search_batched_parallel_newest_optimized_GPT(queries, cluster_centers, primary_inverted_index,
                                           data, k=100, batch_size=20, candidate_chunk_size=100000,
                                           max_streams=4):
    """
    使用GPU流并行化批处理优化搜索，优化包括：
      1. 使用多个GPU流进行并行处理。
      2. 自动调节并发流数，以避免内存占用过高。
    """
    device = queries.device
    nq, d = queries.shape
    num_clusters = cluster_centers.size(0)
    ...
    # 批量处理各 (primary, secondary) 组合
    num_batches = (unique_combos.size(0) + batch_size - 1) // batch_size
    batch_size = min(batch_size, len(unique_combos))

    # 限制流的数量，尝试不同组合的流数
    max_streams = min(max_streams, num_batches)
    ...
    for batch_idx in range(num_batches):
        ...
        # 异步执行：分配工作给不同的流
        stream_idx = batch_idx % max_streams
        ...
        with torch.cuda.stream(stream):
            ...
            # 收集候选集
            for c1, c2 in batch_combos:
                c1_candidates = primary_inverted_index.get(c1.item(), torch.tensor([], device=device, dtype=torch.long))
                c2_candidates = primary_inverted_index.get(c2.item(), torch.tensor([], device=device, dtype=torch.long))
                unique_candidates = merge_sorted_unique(c1_candidates, c2_candidates)
                batch_candidates.append(unique_candidates)
            ...
            for i, (c1, c2) in enumerate(batch_combos):
                query_mask = (inverse_indices == (batch_idx * batch_size + i))
                group_queries = queries[query_mask]
                unique_candidates = batch_candidates[i]

                candidate_vectors = data[unique_candidates]
                nc = unique_candidates.size(0)

                with autocast():
                    scores = torch.matmul(group_queries, candidate_vectors.T)  # shape: (num_queries, nc)

                # 对每个查询获取top-k的候选项
                group_topk_scores, topk_order = torch.topk(scores, k, dim=1)
                group_topk_local_indices = topk_order

                # 对top-k进行填充，确保每个查询都有k个结果
                if nc < k:
                    pad_size = k - nc
                    pad_scores = torch.full((group_queries.shape[0], pad_size), float('-inf'), device=device)
                    pad_indices = torch.full((group_queries.shape[0], pad_size), -1, dtype=torch.long, device=device)
                ...
                # 将局部索引映射回全局索引
                global_indices = unique_candidates[group_topk_local_indices]
                results.append((global_indices, group_topk_scores))
    ...
    all_top_k_distances = []
    for global_indices, group_topk_scores in results:
        all_nearest_neighbors.append(global_indices)
        all_top_k_distances.append(group_topk_scores)

    # 转换为tensor并返回
    all_nearest_neighbors = torch.cat(all_nearest_neighbors, dim=0)
    all_top_k_distances = torch.cat(all_top_k_distances, dim=0)

    return all_nearest_neighbors, all_top_k_distances

def search_cuda_parallel_DeepS(queries, cluster_centers, data_inverted_index, data, k=100,
                        batch_size=128, candidate_chunk_size=50000):
    """
    基于PyTorch CUDA并行原语的优化版本：
    1. 使用批处理矩阵乘法 (bmm) 加速多组查询-候选计算
    2. 自定义核函数优化关键路径
    3. 完全零拷贝的GPU内存操作
    """
    device = queries.device
    ...
    for c1, c2 in unique_combos:
        c1_list = data_inverted_index.get(c1.item(), torch.tensor([], device=device, dtype=torch.long))
        c2_list = data_inverted_index.get(c2.item(), torch.tensor([], device=device, dtype=torch.long))
        merged = merge_sorted_unique(c1_list, c2_list)
        all_candidate_indices.append(merged)

    # 将候选索引转换为掩码矩阵（并行化关键）
    max_candidates = max(len(c) for c in all_candidate_indices) if all_candidate_indices else 0
    candidate_mask = torch.zeros(len(unique_combos), max_candidates, dtype=torch.bool, device=device)
    for i, candidates in enumerate(all_candidate_indices):
        if candidates.numel() > 0:
            candidate_mask[i, :len(candidates)] = True

    # ==== 批处理计算（核心优化部分）====
    all_nearest = torch.full((nq, k), -1, dtype=torch.long, device=device)
    all_scores = torch.full((nq, k), float('-inf'), device=device)

    # 分批次处理组合
    for batch_start in range(0, len(unique_combos), batch_size):
        batch_end = min(batch_start + batch_size, len(unique_combos))
        current_combos = unique_combos[batch_start:batch_end]
        current_masks = candidate_mask[batch_start:batch_end]

        # 收集当前批次的所有候选点
        batch_candidates = torch.cat([all_candidate_indices[i] for i in range(batch_start, batch_end)])
        unique_batch_candidates, reverse_idx = torch.unique(batch_candidates, return_inverse=True)
        candidate_data = data[unique_batch_candidates]  # (total_candidates, d)

        # 构建查询组掩码
        query_masks = [inverse_indices == (batch_start + i) for i in range(batch_end - batch_start)]
        grouped_queries = torch.stack([queries[mask].mean(dim=0) if mask.any() else torch.zeros(d, device=device)
                                       for mask in query_masks])
        valid_mask = torch.tensor([mask.any() for mask in query_masks], device=device)

        # 批处理矩阵乘法（核心优化）
        with autocast():
            scores = torch.zeros(len(current_combos), len(unique_batch_candidates), device=device)
            scores[valid_mask] = torch.matmul(grouped_queries[valid_mask], candidate_data.T).float()

        # 应用候选掩码（过滤无效候选）
        scores[~current_masks[:, :len(unique_batch_candidates)]] = float('-inf')

        # 并行Top-k计算
        topk_scores, topk_indices = torch.topk(scores, k, dim=1)  # 并行处理所有组合

        # 转换到全局索引
        global_indices = unique_batch_candidates[topk_indices]

        # 分发结果
        for i in range(len(current_combos)):
            if valid_mask[i]:
                mask = query_masks[i]
                all_nearest[mask] = global_indices[i].expand(mask.sum(), -1)
                all_scores[mask] = topk_scores[i].expand(mask.sum(), -1)

    return all_nearest, all_scores
...
#     search_batched_chunked_matmul_o(queries, cluster_centers, primary_inverted_index, dataset_normalized,
#                                     k=10))
# tm = timer.end()
# logging.info("search_batched_chunked_matmulo completed !! Elapsed time: {} s".format(tm))
...
# timer.start()
# all_nearest_neighbors1, all_top_k_distances = (
#     search_batched_chunked_matmul_new(queries, cluster_centers, primary_inverted_index, dataset_normalized,
#                                       k=10, batch_size=20))
# tm = timer.end()
# logging.info("search_batched_chunked_matmul_new batch_size=20 completed !! Elapsed time: {} s".format(tm))
# average_recall, recalls = compute_recall_torch(all_nearest_neighbors1, true_neighbors1)
# print(f"Average chunked_matmul Recall: {average_recall}")
# print(recalls)
...
# timer.start()
# all_nearest_neighbors1, all_top_k_distances = (
#     search_batched_chunked_matmul_mixed_precision(queries, cluster_centers, primary_inverted_index,
#                                                   dataset_normalized, k=10, batch_size=20))
#
# tm = timer.end()
# logging.info("search_batched mixed_precision GPT completed !! Elapsed time: {} s".format(tm))
# average_recall, recalls = compute_recall_torch(all_nearest_neighbors1, true_neighbors1)
#
...
timer.start()
# tracer.start()
all_nearest_neighbors3, all_top_k_distances = (
    search_batched_chunked_matmul_mixed_precision_2unique_parallel_GPT(queries, cluster_centers, primary_inverted_index,
                                                                   dataset_normalized, k=10, batch_size=20))
# tracer.stop()
# tracer.save('search_batched_chunked_matmul_mixed_precision_2unique.json')
tm = timer.end()
logging.info(
    "search_batched_chunked_matmul_mixed_precision_2unique_parallel_GPT completed !! Elapsed time: {} s".format(tm))
average_recall, recalls = compute_recall_torch(all_nearest_neighbors3, true_neighbors1)
...
# tracer.start()
all_nearest_neighbors3, all_top_k_distances = (
    search_batched_parallel_newest_optimized_GPT(queries, cluster_centers, primary_inverted_index,
                                                                   dataset_normalized, k=10, batch_size=20))
# tracer.stop()
# tracer.save('search_batched_chunked_matmul_mixed_precision_2unique.json')
tm = timer.end()
logging.info(
    "search_batched_parallel_newest_optimized_GPT completed !! Elapsed time: {} s".format(tm))
average_recall, recalls = compute_recall_torch(all_nearest_neighbors3, true_neighbors1)
...
# # tracer.start()
# all_nearest_neighbors3, all_top_k_distances = (
#     search_cuda_parallel_DeepS(queries, cluster_centers, primary_inverted_index,
#                                                                    dataset_normalized, k=10, batch_size=20))
# # tracer.stop()
# # tracer.save('search_batched_chunked_matmul_mixed_precision_2unique.json')
# tm = timer.end()
# logging.info(
#     "search_cuda_parallel_DeepS completed !! Elapsed time: {} s".format(tm))
# average_recall, recalls = compute_recall_torch(all_nearest_neighbors3, true_neighbors1)
#
# print(f"Average Recall1:  {average_recall}")
# print(recalls)
...
"""现在先只看batch的效果不管chunk"""
# timer = Timer()
# timer.start()
# # tracer.start()
# all_nearest_neighbors, all_top_k_distances = (
#     search_batched2(queries, cluster_centers, primary_inverted_index, dataset_normalized, k=10, batch_size=20))
# # tracer.stop()
# # tracer.save('search_batched2batch_groups.json')
# tm = timer.end()
# logging.info("search_batched2 batch_size=20 completed !! Elapsed time: {} s".format(tm))
# average_recall, recalls = compute_recall_torch(all_nearest_neighbors, true_neighbors1)
#
# print(f"Average Recall2: {average_recall}")
# print(recalls)

# timer = Timer()
# timer.start()
# all_nearest_neighbors1, all_top_k_distances = (
#     search_no_group_batched_but_chunked(queries, cluster_centers, primary_inverted_index, dataset_normalized, k=10))
#
# tm = timer.end()
# logging.info("search_batched without groups batching completed !! Elapsed time: {} s".format(tm))
# average_recall, recalls = compute_recall_torch(all_nearest_neighbors1, true_neighbors1)
#
# print(f"Average Recall1: {average_recall}")
# print(recalls)
